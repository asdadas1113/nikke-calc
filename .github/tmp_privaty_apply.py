...
from fast_engine.tests.test_damage_dynamic_reload_scoring import (
    _capability,
    _member,
)
from fast_engine.engine.model import CompiledEffect, CompiledSquad
...
def _runtime(squad: CompiledSquad, *, duration: float = 10.0) -> BurstRuntime:
    runtime=BurstRuntime(
        squad,
        BurstPolicy(duration=duration, first_burst_time=20.0),
...
class DynamicChargeMaxAmmoSemanticsTests(unittest.TestCase):
    def test_charge_max_ammo_quantizes_each_percentage_source_before_sum(self):
        e0=_max_effect(0,25.0)
        e1=_max_effect(1,25.0)
        squad=_charge_squad((e0,e1),max_ammo=2)
        runtime=_runtime(squad)
        ...
        # Moris: round_half_up(2*25%) + round_half_up(2*25%) = 1+1.
        self.assertEqual(runtime.weapons._full_ammo(0,0.0),4)
        self.assertEqual(runtime.state.ammo[0],4)

    def test_charge_live_cap_drop_clamps_current_ammo_outside_reload(self):
        effect=_max_effect(0,-50.0)
        squad=_charge_squad((effect,),max_ammo=14)
        runtime=_runtime(squad)
        runtime.weapons.start(0.0)
        ...
        self.assertEqual(runtime.state.ammo[0],7)

    def test_charge_reload_finish_uses_live_post_expiry_cap(self):
        effect=_max_effect(0,-50.0,duration=1.5)
        squad=_charge_squad((effect,),max_ammo=14)
        ...
        # Reload timing was already fixed; completion observes the now-expired cap.
        self.assertEqual(st.ammo,14)
    ...
    def test_privaty_first_full_burst_clamps_snow_charge_magazine_like_moris(self):
        ...
        runtime.weapons.start(0.0)
        st=runtime.weapons._states[snow]
        st.ammo=12
        runtime.state.set_ammo(snow,12)
