...
def update_existing_periodic_regression() -> None:
    path = Path("fast_engine/tests/test_damage_periodic_self_crit.py")
...
'''
    if old not in text:
        if new not in text:
            raise SystemExit("periodic self-crit blocker marker missing")
    else:
        text = text.replace(old, new, 1)

    old = '''    def test_other_periodic_shapes_remain_fail_closed(self):
...
'''
    new = '''    def test_other_periodic_shapes_remain_fail_closed(self):
        _moris, compiled, _effect = self._compiled()
        ada = next(
            i for i, m in enumerate(compiled.members)
            if m.name == '에이다'
...
'''
    if old not in text:
        if new not in text:
            raise SystemExit("periodic self-crit neighboring marker missing")
    else:
        text = text.replace(old, new, 1)
    path.write_text(text, encoding="utf-8")


def write_new_regression() -> None:
    path = Path("fast_engine/tests/test_damage_periodic_enemy_received.py")
...
if __name__ == "__main__":
    update_existing_periodic_regression()
    write_new_regression()
