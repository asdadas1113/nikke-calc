...
def update_chained_expectation() -> None:
    path = Path("fast_engine/tests/test_damage_full_charge_hit_charge_speed.py")
    text = path.read_text(encoding="utf-8")
    old = "        self.assertEqual(cadence, 64)\n"
    new = "        self.assertEqual(cadence, 63)\n"
    if old not in text:
        if new in text:
            return
        raise SystemExit("Cinderella cadence expectation marker missing")
    path.write_text(text.replace(old, new, 1), encoding="utf-8")


def write_stat_applied_regression() -> None:
    path = Path("fast_engine/tests/test_damage_stat_applied_charge_speed.py")
...
if __name__ == "__main__":
    update_chained_expectation()
    write_stat_applied_regression()
