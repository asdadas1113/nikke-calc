...
from fast_engine.engine.score import (
    ...
    _charge_actor_score_safe, _rapid_actor_score_safe,
    _rapid_to_single_charge_actor_score_safe,
)
...
print('WAKE', {'id': wake.effect_id, 'stat': wake.stat, 'value': wake.value, 'duration': wake.duration,
    ...
    'conditions': [(r.mode.value, r.key, r.value) for r in wake.condition_rules], 'parameters': dict(wake.parameters)})
...
def diagnostic_shape(effect): return orig_shape(effect) or effect.effect_id == wake.effect_id
...
enemy = EnemyStaticProfile(defense=float(DEFAULT_ENEMY.get('def', 31784.0)), element=DEFAULT_ENEMY.get('code'), core_px=float(DEFAULT_ENEMY.get('core_px', 0.0) or 0.0), duration=DURATION)
