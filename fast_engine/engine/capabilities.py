...
# Single-hit DealForm/state directly consumed by calculator.damage.
...
# Deliberately outside the initial patternless target model. Moris NOP status takes precedence.
FAST_PATTERN_EXCLUDED = frozenset({
...
# Current generic-runtime blockers found by the inventory audit.
SPECIAL_STATS = frozenset({"feather_refresh", "squad_ammo_consume_as"})

_COMMON_FIELDS = frozenset({
    "source", "type", "name", "trigger", "target", "stat", "polarity", "values",
    "fixed_value", "duration", "max_stack", "max_trigger", "scaling", "scaling_ref",
    "trigger_values", "tick_interval", "note", "favorite",
})
...
class EffectCategory(str, Enum):
    ...
    CONTROL = "control"
    MORIS_NOP = "moris_nop"
    FAST_PATTERN_EXCLUDED = "fast_pattern_excluded"
    SPECIAL = "special"
    UNKNOWN = "unknown"


class CapabilityDisposition(str, Enum):
    """How the current Fast runtime should treat one compiled effect."""

    READY = "ready"
    MIRROR_MORIS_NOP = "mirror_moris_nop"
...
class CapabilityProfile:
    """Certified primitives for a concrete Fast runtime revision.

    A structural audit may prove that a generic primitive *could* represent an
    effect; production routing only marks it READY after the primitive is
    explicitly certified here.
    """

    supported_categories: frozenset[EffectCategory] = frozenset()
...
def _impl_status(root: str) -> dict[str, str]:
    text = (Path(root) / "context" / "IMPL-STATUS.md").read_text(encoding="utf-8")
    out: dict[str, str] = {}
...
def moris_status(stat: str | None, *, root: Path = _ROOT) -> str:
    ...
    if stat in impl:
        return impl[stat]
    if stat.startswith((
        "sequential_damage:", "bonus_damage:", "armor_break_damage:",
        "burst_stage_override:", "debuff_immune:",
    )):
        return "✅"
    return "unknown"
...
def classify_effect(effect: dict[str, Any], *, root: Path = _ROOT) -> EffectCategory:
    ...
    raw_stat = effect.get("stat")
    stat = str(raw_stat) if raw_stat is not None else None
    status = moris_status(stat, root=root)

    if "❌" in status or "🚫" in status:
        return EffectCategory.MORIS_NOP
    if typ == "weapon_change":
        return EffectCategory.CADENCE_TIMELINE
    if stat and stat.startswith(("sequential_damage:", "bonus_damage:", "armor_break_damage:")):
        return EffectCategory.DAMAGE_EVENT
    if typ == "damage" or stat in DAMAGE_EVENT_STATS:
        return EffectCategory.DAMAGE_EVENT
    if stat in HIT_FORMULA_STATS:
        return EffectCategory.HIT_FORMULA
    if stat in DERIVED_STATE_STATS:
        return EffectCategory.DERIVED_STATE
...
def timing_family(timing: str) -> str:
    if timing in {"battle_start", "passive"}:
        return "lifecycle"
    if timing.startswith((
        "burst_cast", "conditional_burst_cast_count", "burst_enter:",
        "full_burst_start", "full_burst_end", "squad_burst_cast",
    )):
        return "burst"
    if timing.startswith((
        "hit_count", "conditional_hit_count", "full_charge", "core_hit", "crit_hit",
        "pellet_hit", "multi_hit", "non_full_charge_hit", "charge_hold", "part_hit_count",
        "body_hit_count", "weapon_hit",
    )) or timing in {"on_attack", "last_bullet", "last_bullet_fire"}:
        return "weapon_hit"
    if timing.startswith("every:"):
        return "periodic"
    if timing.startswith("stack_reach:"):
        return "state_counter"
    if timing.startswith(("hp_below", "received_hit", "fatal_hit")):
        return "incoming_hp"
    if timing.startswith("squad_ammo_consume:"):
        return "ammo"
    if timing.startswith("event:"):
        return "named_event"
    if timing in {"enemy_death", "squad_part_break", "squad_part_hit", "feather_tick"}:
        return "encounter_event"
    return "custom"


def condition_family(condition: str) -> str:
    if condition.startswith(("self_state:", "not_self_state:", "target_state:", "not_target_state:")):
        return "named_state"
    if condition.startswith(("self_stack_above:", "target_stack_above:")):
        return "stack"
    if condition.startswith(("gauge_above:", "gauge_below:", "gauge_eq:", "gauge_mod:")):
        return "gauge"
    if condition.startswith(("self_hp_", "ally_hp_")):
        return "hp"
    if condition.startswith("prob:") or condition == "trigger_hit_crit":
        return "rng"
    if condition in {
        "during_full_burst", "not_during_full_burst", "during_charge",
        "burst_casted", "burst_not_casted", "back_row",
    }:
        return "simple_runtime"
    if condition.startswith(("target_code:", "enemy_count_")) or condition == "core_hit":
        return "enemy"
    if condition in {
        "no_burst1_ally", "has_burst1_ally", "no_defender_ally",
        "has_defender_ally", "squad_ally_exists",
    }:
        return "roster"
    if "cover" in condition:
        return "cover"
    if condition in {"during_shield", "target_stunned"}:
        return "control_hp"
    if condition.startswith("self_stat_above:"):
        return "derived_stat"
    if condition == "focusing":
        return "special"
    return "custom"
...
def target_family(target: Any, *, character_names: frozenset[str] = frozenset()) -> str:
    ...
    if not isinstance(target, str):
        return "custom"
    if target in {"self", "all_allies", "all_allies_excl_self"} or target.startswith(("allies:", "allies_adjacent:")):
        return "ally_static"
    if target.startswith((
        "allies_weapon:", "allies_weapon_excl_self:", "allies_class:", "allies_code:",
        "allies_code_weapon:", "allies_code_weapon_leftmost:", "allies_burst3",
        "allies_same_squad", "allies_named:", "all_allies_burst_", "allies_burst_casted_",
        "allies_top_base_charge_time:",
    )):
        return "ally_filter_static"
    if target.startswith((
        "allies_top_atk", "allies_lowest_hp", "allies_top_def", "allies_lowest_atk_burst3",
        "allies_below_def", "allies_weapon_top_atk", "allies_down_top_atk_excl:",
    )):
        return "ally_dynamic_rank"
    if target.startswith((
        "allies_with_buff:", "allies_without_buff:", "allies_burst3_persona",
        "allies_random_debuffed:",
    )):
        return "ally_state_filter"
    if target.startswith("allies_random:"):
        return "ally_random"
    if "cover" in target or target == "all_projectiles":
        return "unsupported_model"
    if target.startswith(("enemy", "enemies", "target", "same_target", "all_enemies")) or target in {
        "target", "enemy", "same_target", "target_body", "target_and_nearby",
    }:
        return "enemy_singleton"
...
def inspect_effect(
    ...
    effect: dict[str, Any],
    *,
    profile: CapabilityProfile = CURRENT_RUNTIME_CAPABILITIES,
    root: Path = _ROOT,
    ...
    if category is EffectCategory.MORIS_NOP:
        disposition = CapabilityDisposition.MIRROR_MORIS_NOP
        blockers: list[str] = []
    elif category is EffectCategory.FAST_PATTERN_EXCLUDED:
        disposition = CapabilityDisposition.MODEL_EXCLUDED
        blockers = []
    elif category is EffectCategory.SPECIAL:
        disposition = CapabilityDisposition.FALLBACK
        blockers = [f"special:{stat or typ}"]
    elif category is EffectCategory.UNKNOWN:
        disposition = CapabilityDisposition.UNKNOWN
        blockers = [f"unknown_stat:{stat}"]
    else:
        blockers = []
        if category not in profile.supported_categories:
        ...
        if target not in profile.supported_target_families:
            blockers.append(f"target:{target}")
        for field in advanced:
            if field not in profile.supported_advanced_fields:
                blockers.append(f"field:{field}")
    ...
    return EffectCapability(
        ...
        stat=stat,
        category=category,
        timing_families=timings,
        condition_families=conditions,
...
def inspect_character_effects(
    ...
    *,
    ...
    root: Path = _ROOT,
    character_names: frozenset[str] = frozenset(),
) -> tuple[EffectCapability, ...]:
    return tuple(
        inspect_effect(
            character, i, effect, profile=profile, root=root, character_names=character_names
        )
