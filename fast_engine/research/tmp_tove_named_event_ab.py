    ...
    text = replace_between(
        ...
        '''            elif stat in {"ammo_charge_pct", "ammo_charge_flat"}:
                ...
                if not self._ammo_charge_sink(stat, actor_targets, value, now):
                    return False
                # Moris' percent-ammo instant handler broadcasts event:{name}
                # after the refill succeeds. Flat refill intentionally does not.
                if (
...
''',
    ...
    if not consumers:
        return True
    # Moris emits a named event only for percent refill. Keep flat refill
    # fail-closed when anything depends on its name.
    if (effect.stat or "") != "ammo_charge_pct":
        return False
    # This first lane only composes the instant event with damage-facing buffs
    # owned by the same actor. Other consumers keep the provider fail-closed.
    return all(
    ...
    for key in keys:
        ...
        for provider in providers:
            if provider.effect_type == "instant":
                # Moris' ammo_charge_pct handler emits event:{name} to the
                # instant caster. Source proof is separate from cadence proof;
                # unsafe refill cadence retains its own blocker.
                if (
...
class TovePctInstantNamedEventABTests(unittest.TestCase):
    def test_synthetic_pct_provider_emits_named_event_and_is_score_safe(self):
        ...
        runtime = BurstRuntime(
            ...
            EnemyStaticProfile(defense=0.0, duration=5.0),
        )
    ...
    def test_real_tove_consumer_source_is_certified_and_runtime_event_fires(self):
        ...
        for actor in range(len(squad.members)):
            self.assertAlmostEqual(
                runtime.dispatcher.effects.sum_stat(actor, "crit_dmg", now=0.91),
                float(followup.value or 0.0),
                places=9,
            )
...
def main() -> None:
    ...
    run("git", "diff", "--check")
    run("git", "diff", "--", "fast_engine/engine/dispatcher.py", "fast_engine/engine/score.py")
